...
! Version 2.3 includes some minor speed enhancements
!
! CHANGES:
...
SUBROUTINE jasmine(bP,p,e,win,aR,Pdays,durexact,&
 ...
 lam2 = (aR*(1.0D0-e**2))**2
 rhoP = (1.0D0-e**2)
 rhoS = rhoP/(1.0D0-e*DSIN(w))
 rhoP = rhoP/(1.0D0+e*DSIN(w))
 bS = (rhoS/rhoP)*bP
 !IF( bS > 1.0D0 ) THEN
 ! IF( verbose == 2 ) THEN
 ...
 ! fP guess
 fPguess = halfpi - w
 IF( fPguess .LT. 0.0D0 ) THEN
  fPguess = fPguess + twopi
 END IF
 ! fS guess
 fSguess = fPguess - pi
 IF( fSguess .LT. 0.0D0 ) THEN
  fSguess = fSguess + twopi
 END IF
...
! = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
 ...
 DO i=1,4
   Dcoeff(i) = Dcoeff(i)/Dcoeff(5)
 END DO

  call quartic(Dcoeff(1),Dcoeff(2),Dcoeff(3),Dcoeff(4),DurComF(1),DurComF(2),DurComF(3),DurComF(4))
  ! Assign real parts
  DO i = 1,4
   DurCosF(i) = DurComF(i)
  END DO
...
! = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
  ...
  DO i=1,4
   ...
   DurF(i,2) = -DACOS(DurCosF(i)) + twopi
  END DO
...
! = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
  ...
  IF( calcapprox == 1 ) THEN
   ...
   delfP = (v(k) - bP*2)/(aR**2*rhoP**2 - bP**2)
   delfP = 2.0D0*DASIN(DSQRT(delfP))
   IF( bP**2 .GT. v(k) ) THEN
    delfP = 0.0D0
   END IF
   ! Secondary eclipse
   delfS = (v(k) - bS**2)/(aR**2*rhoS**2 - bS**2)
...
! = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = 
 ...
 IF( bp .LT. DSQRT(v(3)) .AND. calcpri .EQ. 1 ) THEN

  ! Angular width of tT
  angwidthP = ABS(PriF(1,1) - PriF(2,1))
  IF( angwidthP > pi ) THEN
   angwidthP = 0.5D0*(twopi - angwidthP)
  ELSE
  ...
  DO j=1,2
   DO y=1,2
    ...
    pricombo(j,y) = DABS(PriF(j,3) - PriF(y,1))
    ! Test for a flip
    IF( pricombo(j,y) .GT. pi ) THEN
     pricombo(j,y) = twopi - pricombo(j,y)
    END IF
    IF( vlogfile == 1 ) THEN
     write(11,*) ' '
    ...
    IF( pricombo(j,y) < angwidthP ) THEN
     ...
     slopeangP(j) = DABS(PriF(j,3) - PriF(y,1))
     ! Test for a flip
     IF( slopeangP(j) .GT. pi ) THEN
      slopeangP(j) = twopi - slopeangP(j)
     END IF
     IF( ABS(PriF(j,3)-PriF(y,1)) .LT. pi ) THEN
      ! Normal case, assign slopeang as the addition on top of the lowest angle
      IF( PriF(j,3) .LT. PriF(y,1) ) THEN
     ...
     ELSE
      ...
      ELSE
       slopeangP(j) = PriF(j,3) + slopeangP(j)
      END IF
      IF( slopeangP(j) .GT. twopi ) THEN
       slopeangP(j) = slopeangP(j) - twopi
      END IF
     END IF
    ELSE
     ! REJECTED...
  ...
  IF( DABS(slopeangP(2) - slopeangP(1)) .LT. pi ) THEN
   ! Normal case
   IF( slopeangP(1) .LT. slopeangP(2) ) THEN
...
! ! SubSubSection 2.8.2: Secondary eclipses
 ...
 END IF
 ! Can only be done if we have a full transit
 IF( bs .LT. DSQRT(v(3)) .AND. calcsec .EQ. 1 ) THEN

  ! Angular width of tT
  angwidthS = DABS(SecF(1,1) - SecF(2,1))
  IF( angwidthS .GT. pi ) THEN
   angwidthS = 0.5D0*(twopi - angwidthS)
  ELSE
  ...
  DO j=1,2
   DO y=1,2
    ! Define angular width for each combo
    seccombo(j,y) = DABS(SecF(j,3) - SecF(y,1))
    ! Test for a flip
    IF( seccombo(j,y) .GT. pi ) THEN
     seccombo(j,y) = twopi - seccombo(j,y)
    END IF
    IF( vlogfile == 1 ) THEN
     write(11,*) ' '
    ...
    END IF
    ! Now test if less than the tT width or not
    IF( seccombo(j,y) .LT. angwidthS ) THEN
     ! ACCEPTED...
     IF( vlogfile == 1 ) THEN
      write(11,*) 'Accepted'
     END IF
     ...
     slopeangS(j) = DABS(SecF(j,3) - SecF(y,1))
     ! Test for a flip
     IF( slopeangS(j) .GT. pi ) THEN
      slopeangS(j) = twopi - slopeangS(j)
     END IF
     IF( DABS(SecF(j,3)-SecF(y,1)) .LT. pi ) THEN
      ! Normal case, assign slopeang as the addition on top of the lowest angle
      IF( SecF(j,3) .LT. SecF(y,1) ) THEN
     ...
     ELSE
      ...
      ELSE
       slopeangS(j) = SecF(j,3) + slopeangS(j)
      END IF
      IF( slopeangS(j) .GT. twopi ) THEN
       slopeangS(j) = slopeangS(j) - twopi
      END IF
     END IF
    ELSE
     ! REJECTED...
  ...
  ! Assign S12 and S34
  ! Whatever angle is lowest is assumed to be t12
  IF( DABS(slopeangS(2) - slopeangS(1)) .LT. pi ) THEN
   ! Normal case
   IF( slopeangS(1) .LT. slopeangS(2) ) THEN
...
! to calculate fpri & fsec.
 ...
 errF = (twopi/Pp)*(DSQRT(1.0D0-e**2)/(rhoP**2))*errT ! Error in terms of f
...
! = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
 ...
 IF( DABS(eta) .GT. errF ) THEN
  ...
  IF( DABS(eta) .GT. errF ) THEN
   ! Go to 3rd order
   eta = -prefac*( (-6.0D0*hhP1 + kk**2*(-1.0D0+2.0D0*hh))/&
   ...
   IF( DABS(eta) .GT. errF ) THEN
    ! Go to 4th order
    eta = -prefac*( (-2.0D0*hhP1 + kk**2*(-1.0D0+3.0D0*hh))/(2.0D0*hhP1**4) )*cos2i**4
    fpri = fpri - eta
    IF( vlogfile == 1 ) THEN
    ...
    IF( DABS(eta) .GT. errF ) THEN
     ! Go to 5th order
     eta = prefac*( (40.0D0*hhP1**2-40.0D0*kk**2*(-1.0D0+3.0D0*hh+4.0D0*hh**2) + &
            kk**4*(3.0D0-19.0D0*hh+8.0D0*hh**2))/(40.0D0*hhP1**6) )*cos2i**5
     fpri = fpri - eta
     IF( vlogfile == 1 ) THEN
     ...
     IF( DABS(eta) .GT. errF ) THEN
      ! Go to 6th order
      eta = prefac*( (24.0D0*hhP1**2+9.0D0*kk**4*(1.0D0-8.0D0*hh+5.0D0*hh**2) - &
             40.0D0*kk**2*(-1.0D0+4.0D0*hh+5.0D0*hh**2))/(24.0D0*hhP1**7) )*cos2i**6
      fpri = fpri - eta
      IF( vlogfile == 1 ) THEN
...
! = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
 ...
 IF( DABS(eta) .GT. errF ) THEN
  ...
  IF( DABS(eta) .GT. errF ) THEN
   ! Go to 3rd order
   eta = -prefac*( (-6.0D0*hhM1 + kk**2*(1.0D0+2.0D0*hh))/&
   ...
   IF( DABS(eta) .GT. errF ) THEN
    ! Go to 4th order
    eta = prefac*( (-2.0D0*hhM1 + kk**2*(1.0D0+3.0D0*hh))/(2.0D0*hhM1**4) )*cos2i**4
    fsec = fsec - eta
    IF( vlogfile == 1 ) THEN
    write(11,*) '4th [O] used, 3rd [O] error = ',DABS((eta/errF)*errT),' seconds'
    END IF
    IF( DABS(eta) .GT. errF ) THEN
     ! Go to 5th order
     eta = prefac*( (40.0D0*hhM1**2-40.0D0*kk**2*(-1.0D0+3.0D0*hh+4.0D0*hh**2) + &
            kk**4*(3.0D0+19.0D0*hh+8.0D0*hh**2))/(40.0D0*hhM1**6) )*cos2i**5
     fsec = fsec - eta
     IF( vlogfile == 1 ) THEN
     write(11,*) '5th [O] used, 4th [O] error = ',DABS((eta/errF)*errT),' seconds'
     END IF
     IF( DABS(eta) .GT. errF ) THEN
      ! Go to 6th order
      eta = -prefac*( (24.0D0*hhM1**2+9.0D0*kk**4*(1.0D0+8.0D0*hh+5.0D0*hh**2) - &
             40.0D0*kk**2*(-1.0D0-4.0D0*hh+5.0D0*hh**2))/(24.0D0*hhM1**7) )*cos2i**6
      fsec = fsec - eta
      IF( vlogfile == 1 ) THEN
      write(11,*) '6th [O] used, 5th [O] error = ',DABS((eta/errF)*errT),' seconds'
      END IF
...
! |DFMT| is proportional to |Cos[w]|
 ...
 IF( calcDFMT == 1 ) THEN
  ...
  ! Choose lowest angle
  IF( DABS(PriF(1,2) - PriF(2,2)) .LT. pi ) THEN
   ! Normal case
   IF( PriF(1,2) .LT. PriF(2,2) ) THEN
...
!==================================================================!
 ...
 IF( calcRV .EQ. 1 ) THEN
  ...
  IF( calcmid .EQ. 1 ) THEN
   call duration(e,Pp,fpri,fRVmin,RVoffset)
   IF( DABS(fRVmin-fpri) .LT. pi ) THEN
    ! Normal case
    IF( fRVmin .LT. fpri ) THEN
     RVoffset = - RVoffset
    END IF
   ELSE
    ! Flip case
    IF( fRVmin .GT. fpri ) THEN
     RVoffset = - RVoffset
    END IF
   END IF
  ELSE
  ...
  IF( vlogfile .EQ. 1 ) THEN
   ...
   write(11,*) 'Approximate eqn gives ~ ',-(e*DCOS(w)*Pp)/twopi,' s'
  END IF
  IF( verbose .EQ. 1 ) THEN
   write(*,*) ' '
   write(*,*) 't(RV) - t(transit) = ',RVoffset,' s'
   write(*,*) 'Approximate eqn gives ~ ',-(e*DCOS(w)*Pp)/twopi,' s'
  END IF
...
!==================================================================!
 ...
 IF( calcSECOFF .EQ. 1 ) THEN
  ...
  ! Approximate version
  secpriapprox = DATAN( (e*DCOS(w)) / (DSQRT(1.0D0-e**2)) )
  secpriapprox = secpriapprox + (e*DCOS(w)*DSQRT(1.0D0-e**2))/(1.0D0-e**2*DSIN(w)**2)
  ...
  END IF
  ! Correction
  IF( (secpri/Pp) .LT. 0.5D0 .AND. (secpriapprox/Pp) .GT. 0.5D0 ) THEN
   secpri = Pp - secpri
   IF( vlogfile == 1 ) THEN
    write(11,*) 'Correcting secpri...'
   END IF
  ELSE IF( (secpri/Pp) .GT. 0.5D0 .AND. (secpriapprox/Pp) .LT. 0.5D0 ) THEN
   secpri = Pp - secpri
   IF( vlogfile == 1 ) THEN
    write(11,*) 'Correcting secpri...'
   END IF
...
! an output which Jammi will understand
 ...
 ! Assign rhostar
 rhostar = (Pp/twopi)**2*Grv*(1.0D0/(aR**3))
...
implicit none
 ...
 ! Q0
 Q0 = csc2i*(S2-lam2) + lam2*DCOS(w)**2
 Q0 = Q0**2
 ! Q1
 Q1 = 2.0D0*csc2i*(S2-lam2) + lam2 + lam2*DCOS(2.0D0*w)
 Q1 = 2.0D0*csc2i*e*S2*Q1
 ! Q2
 Q2 = lam2*(csc2i*(-2.0D0+e**2)*S2 - lam2 + 2.0D0*csc2i*lam2)*DCOS(2.0D0*w)
 Q2 = csc2i*e**2*S2*lam2 - lam2**2 + Q2
 Q2 = 2.0D0*csc2i**2*e**2*S2*(3.0D0*S2 - lam2) + Q2
 ! Q3
 Q3 = csc2i*e**2*S2 - lam2*DCOS(2.0D0*w)
 Q3 = 4.0D0*csc2i*e*S2*Q3
 ! Q4
 Q4 = lam2**2 - 2.0D0*csc2i*e**2*S2*lam2*DCOS(2.0D0*w)
 Q4 = csc2i**2*e**4*S2**2 + Q4
 ! Array it
 Q(1) = Q0
 Q(2) = Q1
 Q(3) = Q2
...
implicit none
 ...
 ! R0
 R0 = -4.0D0*e**2*(DCOS(w)**2*sin2i - 1.0D0)**2
 R0 = R0 + sin2i**2*DSIN(2.0D0*w)**2
 ! R1
 R1 = -DCOS(w)**2 + (DCOS(w)**4)*sin2i + DSIN(w)**2
 R1 = -8.0D0*e*sin2i*R1
 ! R2
 R2 = e*e*(3.0D0+cosi2)*DCOS(2.0D0*w) + cosi2*(-2.0D0+e**2)
 R2 = 2.0D0 + 3.0D0*e**2 + R2
 R2 = 4.0D0*(e**2 - 0.25D0*R2*sin2i)
 ! R3
 R3 = -2.0D0*e*(3.0D0 + cosi2)*DCOS(2.0D0*w)*sin2i
 R3 = R3 + 4.0D0*e*sin2i**2
 ! R4
 R4 = 4.0D0*sin2i**2
 ! Array it
 R(1) = R0
...
! ======================================================
SUBROUTINE quartic(a,b,c,d,x1,x2,x3,x4)
...
!
! 0 == a + b x + c x^2 + d x^3 + x^4
! i.e. we explicity assume the coefficients have been normalized
! by the coefficient of x^4
!
...
implicit none
 ...
 COMPLEX(8) :: p, q, r, s ! First level
 COMPLEX(8) :: temp, tempa, tempb ! Second level
 ! Final solutions
 ...
 ! First level
 p = -2.0D0*c**3 + 9.0D0*b*c*d - 27.0D0*a*d**2 
 p = p - 27.0D0*b**2 + 72.0D0*a*c
 q = c**2 - 3.0D0*b*d + 12.0D0*a
 r = p + SQRT( p**2 - 4.0D0*q**3 )
 !write(*,*) 'r = ',r
 !IF( REAL(p**2) .GT. REAL(4.0D0*q**3) ) THEN
 !  r = p + DCMPLX( DSQRT( REAL(p**2 - 4.0D0*q**3) ) )
 !ELSE
 !  r = p + DCMPLX( DSQRT(-REAL(p**2 - 4.0D0*q**3)) )*SQRT(CMPLX(-1))
 !END IF
 r = r**third
 s = ((tom*q)/(3.0D0*r)) + (r/(3.0D0*tom))
 
 ! Second level
 temp = SQRT( d**2 - 8.0D0*third*c - 4.0D0*s )
 tempa = ( 4.0D0*b - 2.0D0*c*d + 0.50D0*d**3 )/temp
 tempb = s + 0.50D0*d**2 - 4.0D0*third*c
 tempb = tempb + tempa       ! pos root = tempb + tempa
 tempa = tempb - 2.0D0*tempa ! neg root = tempb - tempa
 tempa = 0.50D0*SQRT(tempa); tempb = 0.50D0*SQRT(tempb)

 ! x solutions
 x1 = 0.25D0*(-d - temp) - tempb
 x2 = 0.25D0*(-d - temp) + tempb
 x3 = 0.25D0*(-d + temp) - tempa
 x4 = 0.25D0*(-d + temp) + tempa

 ! Check that x is a solution
 !write(*,*) 'Sol x1 gives ',a + b*x1 + c*x1**2 + d*x1**3 + x1**4
 !write(*,*) 'Sol x2 gives ',a + b*x2 + c*x2**2 + d*x2**3 + x2**4
 !write(*,*) 'Sol x3 gives ',a + b*x3 + c*x3**2 + d*x3**3 + x3**4
 !write(*,*) 'Sol x4 gives ',a + b*x4 + c*x4**2 + d*x4**3 + x4**4
 !write(*,*) ' '
